	...
	*/
	if (TriggerFlag == 0)
	{
		...
		else
		{
			...
			for (i = 0; i < DSOSCREEN_LENGTH; i++)
			{
				g_DSO1->usWaveBuf[i] = g_DSO1->usWaveBufTemp[599 - i];
			}
		}
	}
	/* 选择 普通触发 模式后的波形显示 
	...
	else
	{
		...
		if (TriggerFlag == 2)
		{
			...
			*/
			for (i = 724; i < 1324; i++)
			{
				g_DSO1->usWaveBuf[i - 724] = g_DSO1->usWaveBufTemp[i + g_DSO1->sCurTriStep];
			}
...
void DSO2_WaveTrig(uint16_t usCurPos)
{
	...
	*/
	...
	{
		/* 读取的是ADC1的位置 */
		usCurPos = 10240 - DMA2_Stream0->NDTR;
		...
		else
		{
			...
			{
				/* g_DSO2->ulSlowRefresh0表示DMA当前传输到的位置 */
			...
			for (i = 0; i < DSOSCREEN_LENGTH; i++)
			{
				g_DSO2->usWaveBuf[i] = g_DSO2->usWaveBufTemp[599 - i];
			}
		}
	}
}
